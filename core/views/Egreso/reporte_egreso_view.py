...
def reporte_egresos_pdf(request):
    """
    Vista para generar un reporte PDF de egresos en un rango de fechas.
    Parámetros GET:
    ...
    try:
        ...
        # Obtener los egresos y el total en el rango especificado
        egresos = obtener_egresos_rango(fecha_inicio, fecha_fin)
        total = obtener_total_egresos_rango(fecha_inicio, fecha_fin)
        
        # Generar el PDF
        pdf = generar_pdf_egresos(egresos, fecha_inicio, fecha_fin, total, request)
        
        # Devolver el PDF como respuesta HTTP
        response = HttpResponse(pdf, content_type='application/pdf')
        response['Content-Disposition'] = f'filename="reporte_egresos_{fecha_inicio}_{fecha_fin}.pdf"'
        return response
...
@login_required
def reporte_egresos_form(request):
    """
    Vista para mostrar el formulario de generación de reportes de egresos
    """
    form = ReporteEgresoForm(request.GET or None)
    ...
    return render(request, 'egreso/egreso_reporte_form.html', {'form': form})